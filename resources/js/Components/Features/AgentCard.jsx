...
export default function AgentCard({ agent }) {
    ...
    return (
        <div dir={isRtl ? 'rtl' : 'ltr'} className="bg-white rounded-xl shadow-card p-6">
            <Link href={localizedPath(`/agents/${agent.id}`, locale)} className="flex items-center gap-4 mb-4 hover:opacity-80 transition-opacity">
                {avatarSrc ? (
                    <img src={avatarSrc} alt={agent.name} className="w-14 h-14 rounded-full object-cover" loading="lazy" width="56" height="56" />
                ) : (
                    <div className="w-14 h-14 rounded-full bg-primary-100 flex items-center justify-center text-primary-900 font-bold text-lg">
            ...
            <div className="space-y-2">
                {channels.map(ch => (
                    <a
                        ...
                        className="flex items-center gap-2 text-sm text-secondary-700 hover:text-primary-900 transition-colors"
                    >
                        {ch.key === 'phone' && (
