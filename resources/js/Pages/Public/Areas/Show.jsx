...
export default function AreaShow({ area, relatedAreas, units, projects, seo, areas, unitTypes, features, finishingTypes }) {
    ...
    const heroImage = getStorageUrl(area?.hero_image || area?.image_path, '/images/fallback.webp')
                            ...
                            {area?.gallery && area.gallery.length > 0 && (
                                <div className="mt-8 grid grid-cols-2 md:grid-cols-3 gap-4">
                                    {area.gallery.map((img, idx) => (
                                        <img key={idx} src={getStorageUrl(img)} alt={`${areaName} gallery ${idx}`} className="w-full h-32 md:h-40 object-cover rounded-2xl shadow-sm hover:scale-105 transition-transform duration-300 cursor-pointer" />
                                    ))}
                                </div>
                            )}
                        </div>
